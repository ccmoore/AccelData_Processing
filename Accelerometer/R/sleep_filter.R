...
sleep_filter <- function(acceldata_dir, sleeplog_dir, output_dir, 
                         subjectID_length,accelfile_pattern = "^PrepedData",
                         sleeplog_pattern = "*SleepLog", accelfile_IDstart = 12, 
                         sleeplog_IDstart = 1) {
  ...
  for (i in 1:length(accel_files)) {
    ...
    for (j in 1:length(sleeplog_data$Day)) {
      wakeup_time <- as.POSIXlt(paste(sleeplog_data$Date[j], 
                                      sleeplog_data$Wakeup_Time[j]),
        format = "%m/%d/%Y %I:%M:%S %p"
      )
      bed_time <- as.POSIXlt(paste(sleeplog_data$Date[j], 
                                   sleeplog_data$Bed_Time[j]),
        format = "%m/%d/%Y %I:%M:%S %p"
      )
    ...
    print(paste(
      "FilteredWakeData file written in", output_dir, "for subject",
