...
acceldata_prep <- function(input_dir, file_pattern, output_dir,
  ...
  for (i in 1:length(files_1sec)) {
    ...
    # Create timestamp
    acceldata_1sec$TimeStamp <- as.POSIXlt(paste(
      acceldata_1sec$date,
    ...
    acceldata_reint <- dataCollapser(
      dataset = acceldata_1sec,
      TS = "TimeStamp", by = 60
    )

    # Add day variable
    acceldata_days <- markingTime(
      dataset = acceldata_reint,
      timestamp = "TimeStamp",
      startTime = "00:00:00", tz = "UTC"
    )
    # Classify weartime
    acceldata_final <- wearingMarking(acceldata_days,
      frame = 90,
    ...
    write.csv(acceldata_final,
      paste0(
        output_dir, "/PrepedData_",
        substr(files_1sec[i],
          start = subjectID_start,
    ...
    print(paste(
      "PrepedData file written in", output_dir, "for subject",
      substr(files_1sec[i],
        start = subjectID_start,
        stop = (subjectID_start + subjectID_length - 1)
      )
