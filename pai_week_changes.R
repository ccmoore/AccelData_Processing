...
intensity_week_summary <- function(acceldata_PAI) {
  # Classify physical activity intensity by day of the week
  pai_byday <<- acceldata_PAI %>%
    filter(wearing == "w") %>% 
    group_by(weekday, pai) %>% 
    summarise(minutes = n())
  
  # Classify physical activity intensity by weekday compared to weekend
  ...
  # Classify average physical activity intensity for the total week
